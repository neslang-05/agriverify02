...
export function CameraScanner({ 
  ...
  return (
    <div className="flex flex-col h-full min-h-[calc(100vh-4rem)] bg-gradient-to-b from-emerald-800 via-emerald-900 to-emerald-950">
      ...
      <div className="flex-1 flex flex-col lg:flex-row items-center justify-center p-6 lg:p-12 max-w-7xl mx-auto w-full">
        {images.length === 0 ? (
          ...
          <div className="text-center w-full max-w-lg relative">
            ...
            <div className="absolute -bottom-10 -right-10 opacity-10 hidden lg:block">
              <Leaf className="h-32 w-32 text-white rotate-45" />
            </div>
            <div className="absolute top-20 left-4 opacity-10 lg:hidden">
              <Leaf className="h-32 w-32 text-white" />
            </div>
            <div className="absolute bottom-20 right-4 opacity-10 lg:hidden">
            ...
            <div className="mb-8 lg:mb-12 relative z-10">
              ...
              <p className="text-emerald-200/80 text-lg lg:text-xl">
            ...
            {cameraError && (
              <div className="mb-4 p-3 bg-red-500/20 border border-red-400/30 rounded-lg text-red-200 text-sm lg:text-base max-w-md mx-auto">
                {cameraError}
              </div>
            )}

            {/* Desktop: Side by side buttons */}
            <div className="hidden lg:flex gap-6 justify-center mb-8">
              <button
                onClick={startCamera}
                className="group relative w-48 h-48 rounded-2xl bg-white/10 backdrop-blur-sm border-2 border-white/30 flex flex-col items-center justify-center transition-all duration-300 hover:scale-105 hover:bg-white/20 hover:border-white/50"
              >
                <Camera className="h-16 w-16 text-white mb-3 group-hover:scale-110 transition-transform duration-300" />
                <span className="text-white text-lg font-medium">Open Camera</span>
              </button>
              ...
              <button
                ...
                className="group relative w-48 h-48 rounded-2xl bg-white/10 backdrop-blur-sm border-2 border-white/30 flex flex-col items-center justify-center transition-all duration-300 hover:scale-105 hover:bg-white/20 hover:border-white/50"
              >
                <ImageIcon className="h-16 w-16 text-white mb-3 group-hover:scale-110 transition-transform duration-300" />
                <span className="text-white text-lg font-medium">From Gallery</span>
              </button>
            </div>
            ...
            <p className="text-emerald-200/60 text-sm lg:text-base mt-6">
              Upload up to {maxImages} images for accurate results
            </p>
          </div>
        ) : (
          // Images Added - Show grid with add more option
          <div className="w-full max-w-md lg:max-w-2xl bg-white/10 backdrop-blur-sm rounded-xl p-4 lg:p-8">
            <div className="text-center mb-4 lg:mb-6">
              <h2 className="text-xl lg:text-2xl font-semibold text-white">
                {images.length} {images.length === 1 ? 'Image' : 'Images'} Ready
              </h2>
              <p className="text-emerald-200/70 text-sm lg:text-base">
            ...
            <div className="grid grid-cols-3 lg:grid-cols-4 gap-2 lg:gap-4 mb-4 lg:mb-6">
              {images.map((img, index) => (
                <div key={index} className="relative aspect-square">
                  ...
                  <button
                    onClick={() => removeImage(index)}
                    className="absolute top-1 right-1 lg:top-2 lg:right-2 bg-red-600 text-white rounded-full p-1 lg:p-1.5 hover:bg-red-700 transition-colors shadow-lg"
              ...
              {images.length < maxImages && (
                ...
                >
                  <Plus className="h-8 w-8 lg:h-10 lg:w-10 mb-1" />
            ...
            <div className="space-y-3 lg:space-y-4">
              <Button
                onClick={onAnalyze}
                disabled={disabled}
                className="w-full h-14 lg:h-16 rounded-lg bg-white text-emerald-900 hover:bg-emerald-50 text-lg lg:text-xl font-semibold shadow-lg"
              >
                Analyze Seeds
              </Button>
              ...
              {images.length < maxImages && (
                <div className="flex gap-2 lg:gap-4">
                  <button
                    onClick={startCamera}
                    className="flex-1 h-11 lg:h-14 rounded-lg border border-white/40 bg-white/10 text-white font-medium hover:bg-white/20 transition-colors flex items-center justify-center gap-2 lg:text-lg"
                  >
      ...
      <div className="p-4 text-center">
        <p className="text-emerald-200/50 text-xs lg:text-sm">
